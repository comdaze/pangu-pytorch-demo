...
2. 华为 Pangu-Weather 大模型做全球预报（24h 模型自回归逐日步进），采用论文的混合推理（Hybrid Inference）策略：每个自回归步，目标风场变量（10m 风 u10/v10、高空 u/v）取自 VAAWM 微调模型，其余变量（z/q/t、msl/t2m）取自基座 zero-shot 模型，兼顾局地风速精度与全局物理一致性、抑制长程漂移；
