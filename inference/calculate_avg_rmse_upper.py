...
for init_dir in tqdm(forecast_init_dirs, desc="Processing forecast times"):
    csv_path = os.path.join(base_path, init_dir, 'csv', 'rmse_upper_wind_speed.csv')
    ...
    if os.path.exists(csv_path):
        try:
            ...
            if len(df.columns) == 14:
                ...
                if df.columns[0] == '':
                    df.columns = ['date', '1000', '925', '850', '700', '600', '500', '400', '300', '250', '200', '150', '100', '50']
...
if all_forecasts:
    ...
    # 计算每个预报天的平均RMSE
    avg_by_hour = combined_df.groupby('forecast_hour')['850'].agg(['mean', 'std', 'count'])
    
    print("\nAverage RMSE by forecast hour:")
    print(avg_by_hour)
    
    # 保存到CSV
    avg_by_hour.to_csv(f'average_rmse_by_forecast_hour_upper_horizon_{horizon}.csv')
    ...
    plt.ylabel('RMSE (Wind Speed 850)')
    ...
    plt.tight_layout()
    plt.savefig(f'average_rmse_by_forecast_hour_upper_horizon_{horizon}.png', dpi=300)
    # plt.show()
    ...
    if len(forecast_init_dirs) > 1:
        ...
        plt.colorbar(heatmap, label='RMSE (Wind Speed 850)')
        ...
        plt.tight_layout()
        plt.savefig(f'rmse_heatmap_by_init_time_upper_horizon_{horizon}.png', dpi=300)
        # plt.show()
        
        # 保存透视表
        pivot_df.to_csv(f'rmse_by_init_time_and_forecast_hour_upper_horizon_{horizon}.csv')
else:
