...
class PatchEmbedding_pretrain(nn.Module):
    ...
    def check_image_size_2d(self, x):
        _, _, h, w = x.size()
        mod_pad_h = (self.window_size[1] - h %
                     self.window_size[1]) % self.window_size[1]  # 6
        mod_pad_w = (self.window_size[2] - w %
    ...
    def check_image_size_3d(self, x):
        _, _, d, h, w = x.size()
        mod_pad_d = (self.window_size[0] - h %
    ...
    def forward(self, input, input_surface, statistics, maps, const_h):
        ...
        input_surface = torch.permute(
            input_surface, (0, 2, 3, 4, 1))  # [1,1,721,1440,4]
        input_surface = (input_surface - self.surface_mean) / self.surface_std

        input_surface = torch.permute(
            input_surface, (0, 4, 1, 2, 3))  # [1,4 1,721,1440]
        ...
        input = input.reshape(
            input.shape[0], input.shape[1], 1, input.shape[2], input.shape[-2], input.shape[-1])
        input = torch.permute(input, (0, 2, 3, 4, 5, 1))  # [1,1,13,721,1440,5]
        input = torch.flip(input, [2])  # [1,1,13,721,1440,5]
        input = (input - self.upper_mean) / \
            self.upper_std  # [1,1,13,721,1440,5]
        ...
        input = torch.cat((input, const_h.expand(
            batch_size, -1, -1, -1, -1, -1)), dim=1)  # [1,6,1,13,721,1440]
        ...
        input = input.reshape(
            input.shape[0], input.shape[1] * input.shape[2] * input.shape[3] * input.shape[4], -1)
            ...
            if self.use_checkpoint:
                if i % 2 == 0:
                    x = checkpoint.checkpoint(
                        blk, x, Z, H, W, False, use_reentrant=False)
                else:
                    x = checkpoint.checkpoint(
                        blk, x, Z, H, W, True, use_reentrant=False)
            else:
                if i % 2 == 0:
...
class EarthSpecificBlock(nn.Module):
    ...
    '''
        ...
        self.type_of_windows = (input_shape[0]//self.window_size[0])*(
            input_shape[1]//self.window_size[1])  # (8//2*186//6=124) (8//2*96//6=124=64)
    ...
    def forward(self, x, Z, H, W, roll):
        ...
        x = F.pad(x, (0, 0, 0, 0, self.padding_front,
                  self.padding_back), 'constant')
        ...
        if roll:
            ...
            x = torch.roll(x, shifts=[-self.window_size[0]//2, -
                           self.window_size[1]//2, -self.window_size[2]//2], dims=(1, 2, 3))
      ...
      """
        ...
        x_window = x.view(x.shape[0], x.shape[1]//self.window_size[0], self.window_size[0], x.shape[2] //
                          self.window_size[1], self.window_size[1], x.shape[3] // self.window_size[2], self.window_size[2], x.shape[-1])
        ...
        x_window = x_window.contiguous().view(
            x_window.shape[0], x_window.shape[1], self.window_size[0]*self.window_size[1]*self.window_size[2], x_window.shape[-1])
        ...
        x_shifted = attn_windows.view(1, attn_windows.shape[0], Z // self.window_size[0], H // self.window_size[1] + 1,
                                      self.window_size[0], self.window_size[1], self.window_size[2], -1)  # 1,30,4,31,2,6,12,192
        ...
        if roll:
            ...
            x = torch.roll(x_shifted, shifts=[
                           self.window_size[0]//2, self.window_size[1]//2, self.window_size[2]//2], dims=(1, 2, 3))
        else:
            x = x_shifted
...
class EarthAttention3D(nn.Module):
    ...
    '''
        ...
        self.type_of_windows = (
            input_shape[0]//window_size[0])*(input_shape[1]//window_size[1])
    ...
    def forward(self, x, mask):
        ...
        qkv = torch.reshape(x, shape=(
            x.shape[0], x.shape[1], x.shape[2], 3, self.head_number, self.dim // self.head_number))
        # 30，124，144，3，6，32
